...
<button className='btn btn-dark my-3 mx-1  my-1' onClick={toUpCase}>To Upper Case</button>
<button className='btn btn-dark my-3 mx-1  my-1' onClick={toLoCase}>To Lower Case</button>
</div>
    </div>
    <div className='container' style={{color:props.mode==='light'?'black':'white'}}>
    <h2>Your Text Summary</h2>
    <p>Your text has {text.split(" ").filter((ele)=>{return ele.length !=0}).length} words and {text.length} characters</p>
   </div>
   </>
  )
}
